<script lang="ts">
  ...
  $: awaitingConfirmationEdit = !preparedEdit
    ? edits.find((edit) => edit.status === "awaiting_confirmation") ?? null
    : null;
  ...
  $: editModeLocked = Boolean(activeTurn || activeEdit || preparedEdit || editBusy);
  $: canPrepareEdit = Boolean(
  ...
  function changeChatMode(mode: "ask" | "edit") {
    if (editModeLocked || chatMode === mode) return;
    chatMode = mode;
    closeMentionPicker();
  ...
  async function cancelPreparedEdit(editId = preparedEdit?.editId ?? awaitingConfirmationEdit?.editId) {
    if (!window.phaseatlas || !editId || editBusy) return;
    editBusy = true;
    errorMessage = "";
    try {
      await window.phaseatlas.chat.cancelEdit(checkoutId, editId);
      if (preparedEdit?.editId === editId) preparedEdit = null;
      await refreshEdits();
    } catch (error) {
      showError(error);
    } finally {
...
>
  <section class="chat-shell kilo-chat" bind:this={chatShellElement} onpaste={handleComposerPaste}>
    <header class="chat-header">
      ...
      <span class="access-chip" data-mode={chatMode} data-access={editAccessMode}>
        <svg viewBox="0 0 24 24" aria-hidden="true">
          {#if chatMode === "ask"}<rect x="5" y="10" width="14" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/>
          {:else}<path d="M12 3 5 6v5c0 4.6 2.8 8.1 7 10 4.2-1.9 7-5.4 7-10V6z"/><path d="m9.5 12 1.7 1.7 3.7-4"/>{/if}
        </svg>
        {chatMode === "ask" ? "Read only" : editAccessMode === "ask_for_approval" ? "Edit · approval" : "Edit · full access"}
      </span>
      ...
      <main class="conversation">
        {#if selectedSession}
          ...
          <div class="transcript" bind:this={transcript} onscroll={handleTranscriptScroll} aria-live="polite" aria-label="Conversation transcript">
            {#if selectedTurns.length}
              <div class="transcript-column">
                {#each selectedTurns as turn (turn.turnId)}
                  ...
                  <section class="turn" data-status={turn.status} class:turn-running={ACTIVE.has(turn.status)} class:turn-failed={turn.status === "failed" || turn.status === "cancelled" || turn.status === "interrupted"}>
                    {#if prompt}
                      <article class="message user-message chat-bubble" data-role="user">
                    ...
                    {#if timeline.length || ACTIVE.has(turn.status)}
                      <article class="message assistant-message chat-bubble" data-role="assistant">
            ...
            {:else}
              <section class="conversation-empty">
                ...
                <p>{chatMode === "edit" ? "Repository edit" : "Repository conversation"}</p>
                <h3>{chatMode === "edit" ? "Describe the change you want" : "Ask from where the work lives"}</h3>
                <p>{chatMode === "edit" ? "The agent works in an isolated worktree. Nothing reaches the repository until the resulting diff is reviewed and accepted." : "Inspect architecture, trace behavior, compare approaches, or understand a file. Ask mode is read-only and independent from canonical tasks."}</p>
                <div class="suggestions">
                  {#each chatMode === "edit" ? ["Fix the failing tests and explain the changes", "Refactor this area without changing behavior", "Implement the UI described in the attached image"] : ["Map the main runtime boundaries", "Explain the current repository state", "Find the code responsible for streaming"] as suggestion}
          ...
          <footer class="composer-zone">
            ...
            {#if preparedEdit || awaitingConfirmationEdit || activeEdit || reviewEditResult || retainedEditResult}
              <div class="edit-state-dock" aria-label="Edit activity">
                {#if preparedEdit}
                  <section class="edit-confirmation" aria-label="Confirm isolated edit">
                    ...
                    <dl><div><dt>Repository</dt><dd>{preparedEdit.repositoryName} · {preparedEdit.baseRevision.slice(0, 8)}</dd></div><div><dt>Provider</dt><dd>{preparedEdit.runnerId} · {preparedEdit.model}{preparedEdit.reasoningEffort ? ` · ${preparedEdit.reasoningEffort}` : ""}</dd></div><div><dt>Repository access</dt><dd>Entire repository</dd></div><div><dt>Safety boundary</dt><dd>Isolated worktree · network blocked</dd></div></dl>
                    <p>The agent can start after this approval. Applying the resulting diff always requires a separate review.</p>
                    <div><button type="button" onclick={() => cancelPreparedEdit()} disabled={editBusy}>Cancel request</button><button class="confirm-edit" type="button" onclick={confirmEdit} disabled={editBusy}>{editBusy ? "Starting…" : "Approve and start"}</button></div>
                  </section>
                {:else if awaitingConfirmationEdit}
                  <section class="edit-awaiting" aria-live="polite"><span>!</span><p><strong>An edit request is still waiting for approval</strong><small>The app reloaded before approval. Cancel this stale request, then submit the change again.</small></p><button type="button" onclick={() => cancelPreparedEdit(awaitingConfirmationEdit!.editId)} disabled={editBusy}>{editBusy ? "Cancelling…" : "Cancel request"}</button></section>
                {/if}
                {#if activeEdit}
                ...
                <div class="composer-toolbar">
                  <form class="attachment-entry" onsubmit={(event) => { event.preventDefault(); addAttachment(); }}>
                    <div class="composer-mode-switch" aria-label="Agent mode">
                      <button class:active={chatMode === "ask"} type="button" aria-pressed={chatMode === "ask"} onclick={() => changeChatMode("ask")} disabled={editModeLocked}>Ask</button>
                      <button class:active={chatMode === "edit"} type="button" aria-pressed={chatMode === "edit"} onclick={() => changeChatMode("edit")} disabled={editModeLocked}>Edit</button>
                    </div>
                    ...
                    {#if chatMode === "edit"}
                      <label class="edit-access-select" data-mode={editAccessMode} title="Applies to Edit mode in every repository">
                        ...
                        <select aria-label="Edit access mode" value={editAccessMode} onchange={changeEditAccessMode} disabled={editModeLocked}>
...
<style>
  ...
  .conversation-controls { display: flex; min-width: 0; align-items: center; gap: 7px; }.task-handoff { border: 1px solid var(--border); border-radius: var(--radius); padding: 6px 8px; background: var(--surface); color: var(--text-muted); font-size: 11px; font-weight: 750; }.task-handoff:hover { border-color: var(--brand-300); color: var(--active-text); }.task-handoff:disabled { opacity: .45; }
  ...
  .edit-retained { display: flex; width: 100%; align-items: center; gap: 8px; margin: 0; border: 1px solid var(--warning-500); border-radius: 13px; padding: 10px 11px; background: var(--warning-surface); }.edit-retained p { min-width: 0; flex: 1; margin: 0; }.edit-retained strong,.edit-retained small { display: block; }.edit-retained strong { font-size: 12px; }.edit-retained small { margin-top: 3px; color: var(--text-muted); font-size: 11px; }.edit-retained button { min-height: 30px; border: 1px solid var(--border); border-radius: 9px; padding: 0 9px; background: var(--surface); color: var(--text); font-size: 11px; font-weight: 750; }.edit-retained .resume-edit { border-color: var(--brand-600); background: var(--brand-600); color: white; }
  .edit-verification { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; margin: 9px 0; }.edit-verification > span { border-left: 3px solid var(--success-500); border-radius: var(--radius-xs); padding: 7px 8px; background: var(--surface-soft); }.edit-verification > span[data-status="failed"] { border-left-color: #c44242; }.edit-verification strong,.edit-verification small { display: block; }.edit-verification strong { font-size: 11px; }.edit-verification small { margin-top: 3px; color: var(--text-subtle); font-size: 11px; }
  ...
  .kilo-chat { --kilo-message-radius: 14px; }
  ...
  .kilo-chat .session-item { border-radius: 12px; }
  .kilo-chat .session-item.active { box-shadow: none; }
  .kilo-chat .session-select { border-radius: 10px; padding-right: 50px; }
  .kilo-chat .conversation { grid-template-rows: 56px minmax(0,1fr) auto; }
  .kilo-chat .conversation-header { height: 56px; padding-left: 20px; padding-right: 20px; border-bottom: 1px solid color-mix(in srgb,var(--border) 82%,transparent); }
  .kilo-chat .conversation-header h3 { font-size: 13px; }
  .kilo-chat .conversation-meta span { border-radius: 999px; padding: 5px 9px; background: color-mix(in srgb,var(--surface) 90%,var(--canvas)); }
  .kilo-chat .transcript-column { width: min(920px,calc(100% - 34px)); padding-top: 24px; padding-bottom: 44px; }
  .kilo-chat .turn { position: relative; }
  .kilo-chat .turn + .turn { margin-top: 26px; padding-top: 26px; }
  .kilo-chat .message { border: 0; border-radius: var(--kilo-message-radius); background: var(--surface); }
  .kilo-chat .message header { margin-bottom: 10px; }
  .kilo-chat .message.user-message.chat-bubble { max-width: 77%; margin-left: auto; border-top-left-radius: 4px; background: color-mix(in srgb,var(--active-surface) 74%,var(--surface)); }
  .kilo-chat .message.assistant-message.chat-bubble { border-top-right-radius: 4px; background: color-mix(in srgb,var(--surface) 96%,var(--canvas)); }
  .kilo-chat .message p { font-size: 14px; line-height: 1.68; }
  .kilo-chat .message code { font-size: 12px; }
  .kilo-chat .turn-timeline { gap: 10px; }
  .kilo-chat .timeline-activity { border: 1px solid color-mix(in srgb,var(--border) 74%,transparent); background: var(--surface); border-radius: 10px; }
  .kilo-chat .turn-terminal { border-radius: 12px; }
  .kilo-chat .composer-zone { border-top: 1px solid color-mix(in srgb,var(--border) 82%,transparent); }
  .kilo-chat .composer-card { border-radius: 16px; border: 1px solid color-mix(in srgb,var(--border) 70%,transparent); }
  .kilo-chat .composer-toolbar { min-height: 42px; }
  .kilo-chat .send-button { border-radius: 11px; min-height: 30px; }
  .kilo-chat .conversation-empty h3 { font-size: 20px; }
  .kilo-chat .turn-running::after { pointer-events: none; content: ""; position: absolute; inset: 0; border-radius: 22px; background: linear-gradient(120deg,color-mix(in srgb,var(--brand-500) 8%,transparent),transparent); opacity: .5; }
  .kilo-chat .turn-running::before { pointer-events: none; content: ""; position: absolute; top: 0; left: 0; width: 6px; height: 100%; border-radius: 22px 0 0 22px; background: var(--brand-500); opacity: .28; }
  .kilo-chat .turn-failed .assistant-message.chat-bubble,
  .kilo-chat .turn-failed .user-message.chat-bubble { background: color-mix(in srgb,#c44242 8%,var(--surface)); }
