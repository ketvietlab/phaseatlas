<script lang="ts">
  ...
  function buildMap(currentTasks: CanonicalTask[]): TaskMapModel {
    ...
    const mapHeight = MAP_PADDING * 2 + largestColumn * NODE_HEIGHT + (largestColumn - 1) * ROW_GAP;
    const nodes: MapNode[] = [];

    for (let columnIndex = 0; columnIndex < columnCount; columnIndex += 1) {
      const columnTasks = columns.get(columnIndex) ?? [];
      const columnHeight = columnTasks.length * NODE_HEIGHT + Math.max(columnTasks.length - 1, 0) * ROW_GAP;
      const topOffset = MAP_PADDING + Math.max((mapHeight - MAP_PADDING * 2 - columnHeight) / 2, 0);
      columnTasks.forEach((task, rowIndex) => {
        nodes.push({
          key: taskKey(task),
          task,
          x: MAP_PADDING + columnIndex * (NODE_WIDTH + COLUMN_GAP),
    ...
    for (const target of nodes) {
      for (const dependency of target.task.dependencies.filter((item) => BLOCKING_RELATIONS.has(item.relation))) {
        ...
        if (!source) continue;
        const spansMultipleColumns = target.x - source.x > NODE_WIDTH + COLUMN_GAP + 1;
        edges.push({
    ...
    return {
      nodes,
      edges,
      width: MAP_PADDING * 2 + columnCount * NODE_WIDTH + Math.max(columnCount - 1, 0) * COLUMN_GAP,
      height: mapHeight,
      summary,
    };
  }
  ...
  function edgePath(edge: MapEdge): string {
    ...
    if (edge.routeLane !== undefined) {
      const exitX = startX + Math.max(COLUMN_GAP * 0.34, 28);
      const entryX = endX - Math.max(COLUMN_GAP * 0.34, 28);
      const laneY = 16 + (edge.routeLane % 4) * 10;
      const curveReach = Math.min(COLUMN_GAP * 0.58, 54);
      const laneLead = 34;
      return [
        `M ${startX} ${startY}`,
...
<section class="task-map-shell" aria-label="Task dependency map">
  <header class="map-header">
    <div>
      <p class="map-eyebrow">Dependency atlas</p>
      <h2>Task map</h2>
      <p>Read delivery from left to right. Dashed amber routes are waiting for an upstream task.</p>
    </div>
...
<style>
  .task-map-shell { overflow: hidden; border: 1px solid var(--border); border-radius: var(--radius-lg); background: var(--surface); box-shadow: var(--shadow-card); }
  ...
  .map-summary [data-tone="done"] strong { color: var(--success-text); }.map-summary [data-tone="blocked"] strong { color: var(--warning-600); }.map-summary [data-tone="active"] strong { color: var(--active-text); }
  ...
  .map-legend [data-tone="done"] i { background: var(--success-500); }.map-legend [data-tone="active"] i { background: var(--brand-500); }.map-legend [data-tone="ready"] i { box-shadow: inset 0 0 0 2px var(--brand-500); background: transparent; }.map-legend [data-tone="waiting"] i { background: var(--warning-500); }
  .map-scroll { max-width: 100%; overflow: auto; border-bottom: 1px solid var(--border); background-color: var(--canvas); background-image: linear-gradient(var(--border-soft) 1px, transparent 1px), linear-gradient(90deg, var(--border-soft) 1px, transparent 1px); background-size: 24px 24px; overscroll-behavior: contain; }
  ...
  .task-edge { fill: none; stroke: var(--brand-300); stroke-linecap: round; stroke-linejoin: round; stroke-width: 2.15; vector-effect: non-scaling-stroke; transition: opacity .16s ease, stroke .16s ease; }.task-edge.blocked { stroke: var(--warning-500); stroke-dasharray: 8 7; }.task-edge.complete { stroke: var(--success-500); }
  .task-node { position: absolute; z-index: 1; display: grid; grid-template-rows: auto minmax(34px, auto) auto 1fr; align-content: start; gap: 6px; overflow: hidden; border: 1px solid var(--border); border-radius: var(--radius-lg); padding: 13px 14px 11px; background: color-mix(in srgb, var(--surface) 97%, transparent); color: var(--text); box-shadow: var(--shadow-card); text-align: left; transition: border-color .16s ease, box-shadow .16s ease, opacity .16s ease, transform .16s ease; backdrop-filter: blur(8px); }
  .task-node:hover { z-index: 3; border-color: var(--brand-300); box-shadow: var(--shadow-hover); transform: translateY(-2px); }.task-node.selected { z-index: 2; border-color: var(--brand-500); box-shadow: 0 0 0 3px color-mix(in srgb, var(--brand-300) 30%, transparent), var(--shadow-hover); }
  .task-node[data-tone="done"] { border-color: color-mix(in srgb, var(--success-500) 65%, var(--border)); background: var(--success-surface); }.task-node[data-tone="blocked"],.task-node[data-tone="waiting"] { border-color: color-mix(in srgb, var(--warning-500) 75%, var(--border)); background: var(--warning-surface); }.task-node[data-tone="active"],.task-node[data-tone="review"] { border-color: color-mix(in srgb, var(--brand-500) 70%, var(--border)); background: var(--active-surface); }.task-node[data-tone="ready"] { border-color: var(--brand-300); }
  .node-topline,.node-foot { display: flex; align-items: center; justify-content: space-between; gap: 8px; }.node-id { display: inline-grid; min-width: 58px; height: 24px; place-items: center; border-radius: var(--radius-sm); padding: 0 7px; background: var(--surface-raised); color: var(--active-text); font: 800 11px/1 "SFMono-Regular",Consolas,monospace; letter-spacing: .04em; }.node-state { display: inline-flex; align-items: center; gap: 5px; color: var(--text-muted); font-size: 11px; font-weight: 750; text-transform: uppercase; }.node-state i { width: 7px; height: 7px; border-radius: 50%; background: var(--ink-400); }.task-node[data-tone="done"] .node-state i { background: var(--success-500); }.task-node[data-tone="active"] .node-state i,.task-node[data-tone="review"] .node-state i,.task-node[data-tone="ready"] .node-state i { background: var(--brand-500); }.task-node[data-tone="blocked"] .node-state i,.task-node[data-tone="waiting"] .node-state i { background: var(--warning-500); }
  .task-node > strong { display: -webkit-box; overflow: hidden; font-size: 14px; font-weight: 750; line-height: 1.35; line-clamp: 2; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }.node-meta { overflow: hidden; color: var(--text-subtle); font-size: 11px; text-overflow: ellipsis; text-transform: capitalize; white-space: nowrap; }.node-foot { align-self: end; border-top: 1px solid color-mix(in srgb, var(--border) 72%, transparent); padding-top: 7px; color: var(--text-subtle); font-size: 11px; }.node-foot > span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.node-foot > span:first-child { min-width: 0; }.node-foot > span:last-child { flex: 0 0 auto; }.blocked-by { color: var(--warning-600); font-weight: 700; }.body-ready { color: var(--success-text); }.body-initializing { display: inline-flex; align-items: center; gap: 6px; color: var(--active-text); font-weight: 750; }.content-spinner { display: inline-block; width: 13px; height: 13px; flex: 0 0 13px; border: 2px solid color-mix(in srgb,currentColor 28%,transparent); border-top-color: currentColor; border-radius: 50%; animation: task-map-spin .75s linear infinite; }
